...
export function App() {
    ...
    startTransition(() => {
      operations.forEach((operation) => {
        if (operation.type === 'add-object') {
          addObject(operation.kind, operation.preset);
        }
  ...
  const sendChat = async (message: string) => {
    ...
    if (!trimmed) return;
    const nextThread: ChatMessage[] = [...activeChatMessages, { id: crypto.randomUUID?.() ?? `chat-${Date.now()}`, role: 'user', text: trimmed }];
    ...
    dispatch({
      ...
      messages: [...nextThread, { id: crypto.randomUUID?.() ?? `chat-${Date.now()}-assistant`, role: 'assistant', text: reply.text }],
    });
    setChatBusy(false);
  };
  ...
  return (
    <main className="app-shell">
      <TopBar
        ...
        onToggleRedaction={() => setRedactionOpen((value) => !value)}
      ...
      <section className={`workspace-grid ${!aiOpen ? 'no-chat' : ''} ${sidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
        <SlideRail
          ...
          activeSlideId={editorState.present.activeSlideId}
          collapsed={sidebarCollapsed}
          onToggleCollapsed={() => setSidebarCollapsed((value) => !value)}
          onAddSlide={() => dispatch({ type: 'add-slide' })}
          onSelectSlide={(slideId) => dispatch({ type: 'select-slide', slideId })}
          onReorderSlides={(event: DragEndEvent) => {
        ...
        {aiOpen ? (
          <aside className="right-column">
            <PropertiesPanel
              ...
              onPatchObject={(patch) => selectedObject && dispatch({ type: 'update-object', objectId: selectedObject.id, patch })}
...
function focusBounds({
  ...
  if (adjustZoom && Math.abs(clampedZoom - currentZoom) > 0.001) {
    dispatchZoom(clampedZoom);
  }

  requestAnimationFrame(() => {
    centerOnBounds(bounds, clampedZoom, node);
  });
}
...
function getBoundsZoom(bounds: { width: number; height: number }, node: HTMLDivElement) {
  ...
  return Math.min(
    ...
    Math.max(
      ZOOM_MIN,
      Math.min(
        (node.clientWidth - horizontalPadding) / Math.max(bounds.width, 240),
        (node.clientHeight - verticalPadding) / Math.max(bounds.height, 180),
      ),
    ),
  );
}

function centerOnBounds(bounds: { left: number; top: number; width: number; height: number }, zoom: number, node: HTMLDivElement) {
